...
consistent. Figure~\ref{fig:verdicts} shows the per-block breakdown.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fig_verdicts.pdf}
\caption{Verdict composition of every measured coefficient, by block, from the
independent verification pass. ``Consistent'' coefficients reproduce their
...
calorimeter resolution is essentially flat.

\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{fig_eres.pdf}
\caption{Electron fractional resolution. The card (dashed) applies the tracking
form of Eq.~\ref{eq:momres}, whose $b\,p_\mathrm{T}$ term makes the resolution
diverge at high $p_\mathrm{T}$; the measured CMS electron resolution~\cite{cms-egamma}
(bands) is approximately flat at $\sim$2\% (barrel) and $\sim$4--5\%
(endcap). The $b\,p_\mathrm{T}$ term is a copy of the charged-hadron tracker
coefficients and should be removed for electrons.}
\label{fig:eres}
\end{figure}
...
\midrule
\multicolumn{4}{@{}l}{\footnotesize Electron mom.\ res.\ (constant term; remove $b\,p_\mathrm{T}$; Run-2 sources)}\\
\quad $a$ (ATLAS) & $|\eta|\!\le\!0.5$ & $0.03$ & $0.010$\\
\quad $a$ (CMS)   & $|\eta|\!\le\!0.5$ & $0.03$ & $0.020$\\
\midrule
...
Fig.~9~\cite{cms-muon} (RMS of $R(q/p_\mathrm{T})$, $|\eta|<0.9$, cosmic rays).
Right: \textsc{Delphes} ATLAS barrel curve on ATLAS Fig.~2~\cite{atlas-muon13fullrun2}
($q/p$ resolution vs.\ $p_\mathrm{T}$, $|\eta|<1.05$).}
\label{fig:ovl-cmsmu}
\end{figure}
...
are added to this appendix as their overlays are produced.}
...
the CMS misidentification-vs-efficiency ROC, Fig.~3~\cite{cms-tau}; it sits on the
measured MVA working-point curve.}
\label{fig:ovl-tag}
\end{figure}
...
ATLAS Fig.~8~\cite{atlas-egamma} (vs.\ $\eta$); the card $0.95/0.85$ tracks the
Loose working point.}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{aux_overlay_onfigure_cmseid.pdf}
\caption{CMS electron identification efficiency on CMS Fig.~26~\cite{cms-egamma}
(cut-based Veto working point vs.\ $E_\mathrm{T}$, four $\eta$ ranges). The card
central value $0.95$ matches the Veto plateau; the card forward value $0.85$ lies
well below the measured $\sim$0.95 (the WPs are $\eta$-flat) --- i.e.\ the forward
electron identification in the stock card is too pessimistic.}
