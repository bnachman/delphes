...
Muon mom.\ res.\ (pilot)       & accept       & central floor $1.0\%$ optimistic vs.\ $1.7$--$2.3\%$~\cite{atlas-muon13}\\
...
Jet energy scale               & accept$^\ast$ & \texttt{ScaleFormula} coefficients unsupported (JER~\cite{atlas-jes} needed)\\
b-tagging                      & accept$^\ast$ & 70\% WP eff.\ + mistag consistent, but eff.\ falls too steeply at high $p_\mathrm{T}$\\
...
Jet energy scale               & \textbf{accept} & generic form; no peer-reviewed Run-2 source (cf.\ 8~TeV~\cite{cms-jes})\\
...
$0.85$ (forward) sits above the measured~\cite{atlas-tracking} central-value of
$\sim$0.86 and the forward $\sim$0.73. (The value adopted in the baseline card,
$0.91$ central, is from the simulation-based Loose-track note ATL-PHYS-PUB-2015-051,
$\sim$5\% higher than the min-bias-data central plateau of~\cite{atlas-tracking};
the source-dependence at this level is itself a documented caveat.) The flat
$0.95/0.85$ identification efficiencies do
...
\quad ATLAS ch.-had.\ track & central plateau & $0.95$ & $0.91$ (PUB-2015-051, sim.)\\
\quad ATLAS ch.-had.\ track & forward ($|\eta|\!\sim\!2.5$) & $0.85$ & $0.73$ (PUB-2015-051)\\
...
assumed. (In the first extraction pass, three CMS blocks --- b-tagging, electron
momentum resolution, and $\tau$-tagging --- lost their verification stage to a
workflow interruption and were re-verified in a separate pass; all three returned
\textsc{accept}. The later re-examination pass had its own, distinct interruption
affecting three different blocks; both are recorded in the provenance directory. So the
...
binning, both bracketing the simulated plateau and forward fall-off (this CMS
figure is a simulation result).}
