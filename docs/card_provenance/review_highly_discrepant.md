...
| 3 | Charged-hadron mom. res. **ATLAS** ($a$,$b$) | all | 0.06–0.25 / 1.3–3.1e-3 | **shipped:** $\sqrt{0.013^2+(3.6\!\times\!10^{-4}p_T)^2}\times\eta$-shape | ID TDR $p_T$-shape + PERF-2015-10 figaux (physicist notebook) | **Canonical = the fine-$\eta$ form now in the baseline card** (`apply_user_tracking.py`). The coarse 0.015/0.025/0.040 set was an earlier 3-bin proposal, superseded. Charged hadrons are ID-only → ID slope used (not combined-muon $b$≈1e-4). |
